...
int init_sound (void)
{
    ...
    if (rate < sound_rate * 90 / 100 || rate > sound_rate * 110 / 100) {
	fprintf (stderr, "Can't use sound with desired frequency %d\n", sound_rate);
	return 0;
    }

    bufsize = rate * sound_latency * (dspbits / 8) * (mainMenu_soundStereo ? 2 : 1) / 1000;
	...
	if (!(formats & AFMT_S16_NE))
	    return 0;
	//init_sound_table16 ();
	sample_handler = mainMenu_soundStereo ? sample16s_handler : sample16_handler;
    } else {
	if (!(formats & AFMT_U8))
	    return 0;
	//init_sound_table8 ();
	sample_handler = mainMenu_soundStereo ? sample8s_handler : sample8_handler;
    }
    sound_available = 1;
